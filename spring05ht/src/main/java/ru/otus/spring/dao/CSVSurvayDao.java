...
public class CSVSurvayDao implements SurvayDao {
    ...
    private List<List<String>> getCSVRecords() {
        String resourceName = getCSVLocalizedResourceName();
        try (InputStream is = CSVSurvayDao.class.getResourceAsStream("/" + resourceName)) {
            if (is != null) {
    ...
    private String getCSVLocalizedResourceName()  {
        if (properties.getLocale() != null) {
            ...
            try (InputStream is = CSVSurvayDao.class.getResourceAsStream("/" + csvLocaleFileName)) {
                if (is == null) {
                    ...
                    csvLocaleFileName = properties.getFile();
                }
            } catch (IOException e) {
                LOGGER.error(e.getMessage(), e);
                csvLocaleFileName = "i18n/" + properties.getFile().substring(0, properties.getFile().indexOf('.')) + "_ru.csv";
            }
            return csvLocaleFileName;
        }
